...
if ($ValidateMarkdownHelp)
{
    ...
    Test-AbMarkdownHelp "$PSScriptRoot\..\docs\help" $SuppressedExceptionsPath $NewExceptionsPath

    $Exceptions = Import-Csv "$NewExceptionsPath\ValidateHelpIssues.csv"
    
    if (($Exceptions | Measure-Object).Count -gt 0)
    {
        $Exceptions | Format-Table
...
if ($GenerateMamlHelp)
{
    New-AbMamlHelp -HelpFolderPath "$PSScriptRoot\..\docs\help" -OutputPath "$PSScriptRoot\..\artifacts\$BuildConfig"
}